...
### DVOA Find and Munge ###
###########################
#u <- "http://www.footballoutsiders.com/dvoa-ratings/2017/2017-dvoa-projections"
...
lookup <- df2 %>% select (TEAM,OFFENSEDAVE) %>%
...
m0  <- sc2 %>%
      ...
      # mutate(WEEK = ifelse(TEAM=="TB"  & WEEK == 1, 11, WEEK)) %>%
      # mutate(WEEK = ifelse(TEAM=="MIA" & WEEK == 1, 11, WEEK)) %>%
       ...
       mutate(OFFENSEDAVE = ifelse(is.na(OFFENSEDAVE),0,OFFENSEDAVE)) 

m1   <- m0 %>% select(TEAM,WEEK,OFFENSEDAVE) %>%
        spread(WEEK,OFFENSEDAVE)
m1.5 <- m1 %>% select(TEAM)
